...
def main() -> int:
    ...
    (output / "README.md").write_text(
        "# @0230am/blob-emoji\n\n"
        "Static Blob Emoji assets for Clover. This package contains data and artwork only—"
        "it has no JavaScript entry point, runtime dependencies, or install scripts.\n\n"
        "## Install\n\n"
        "```sh\n"
        f"pnpm add @0230am/blob-emoji@{EMOJIBASE_VERSION}-{commit[:8]}.0\n"
        "```\n\n"
        "Pin the exact version: every release is immutable and tied to one source commit.\n\n"
        "## Bundle metadata\n\n"
        f"- Bundle ID: `{bundle_id}`\n"
        f"- Unicode Emoji: `{EMOJI_VERSION}`\n"
        f"- Emojibase: `{EMOJIBASE_VERSION}`\n"
        f"- Public base URL: `https://static.0230.am/clover/emoji/v1/{bundle_id}/`\n\n"
        "## Contents\n\n"
        "- `manifest.json` — bundle entry point and asset contracts\n"
        "- `clover-picker.json` — compact Clover picker data\n"
        "- `picker.json` — full source picker data\n"
        "- `text-manifest.json` — accessible labels by Unicode sequence\n"
        "- `assets.json` — SVG lookup rules and missing-artwork report\n"
        "- `atlas/` — 64 px picker atlases\n"
        "- `svg/` — sanitized individual artwork\n"
        "- `checksums.sha256` — SHA-256 for every bundled file\n"
        "- `LICENSES/` — upstream attribution and license notices\n\n"
        "## Asset lookup\n\n"
        "Convert the Unicode sequence to lowercase hexadecimal code points joined by `-`, "
        "remove `fe0f`, and read `svg/<asset-key>.svg`. For example, `1F44D-FE0F` resolves "
        "to `svg/1f44d.svg`.\n\n"
        "Country flags use the included waved flag artwork; render the SVG instead of native "
        "regional-indicator text. Atlas slots follow the row order in `clover-picker.json`.\n\n"
        "## Integrity and licensing\n\n"
        "The release pipeline verifies Unicode coverage, sanitizes SVGs, validates atlas slots, "
        "and checks every packaged file against `checksums.sha256`. See `LICENSES/` for the "
        "preserved upstream and Emojibase notices.\n",
        encoding="utf-8",
    )
